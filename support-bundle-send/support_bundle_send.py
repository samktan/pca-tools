...
def get_support_bundles(sourcedir):
    ...
    support_bundles = sorted(support_bundles, key=lambda k: k['timestamp'])
...
def prompt_for_support_bundle(support_bundles):
    ...
    for i, support_bundle_entry in enumerate(support_bundles, start=1):
        ...
        size_string = format_size_to_string(f_size)
    ...
    choice_input = input("Enter support bundle #: [0 to exit]: ")
...
def perform_upload(upload_details):
    ...
    mos_url = f"https://transport.oracle.com/upload/issue/{bundle_sr}/"
    ...
    with open(bundle_path, "rb") as bundle_file:
        response = requests.post(mos_url, data=bundle_file, stream=True, auth=(mos_login, mos_password))
    print(response.text)


def main():
    SUPPORT_BUNDLES_FOLDER="/nfs/shared_storage/support_bundles/"
    # SUPPORT_BUNDLES_FOLDER="C:\\Users\\samktan\\Downloads\\"
